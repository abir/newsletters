...
####################################################################################################
	...
	const MAIL='newsletter_messages';
...
####################################################################################################
	...
	//for select only
	function _is_draft(){return $this->db->where('date_sent','0000-00-00 00:00:00')->where('active',1);}
	function _is_sent(){return $this->db->where('date_sent !=','0000-00-00 00:00:00')->where('active',1);}
...
####################################################################################################
	...
	function edit_newsletter($id=false)
	{
		...
		return $this->db->$action(self::MAIL,$data) ? true:false;
	}
...
####################################################################################################
	...
	function recipients($id=false,$group_id=false,$limit=false,$offset=false)
	{
		...
		elseif($group_id){
			...
			$this->db->join(self::USERS_GROUPS, self::USERS.'.id = '.self::USERS_GROUPS.'.user_id');
		}
...
####################################################################################################
	...
	function edit_recipient($id=false)
	{
		...
		elseif(is_email($email,true) and !empty($name) and !empty($groups))
		{
			...
			$this->db->delete(self::USERS_GROUPS,array('user_id'=>$id)); //delete existing links
			$data2['user_id']=$id;
		...
		if($table=='recipients')
		{
			...
			foreach($ids as $id)
			{
				foreach($this->recipients($id) as $item)
				{
					...
					$this->db->where('user_id',$id)->delete(self::USERS_GROUPS);
					$items[] = $item->email;
				}
			}
		}
		...
		elseif($table=='newsletters')
		{
			foreach($ids as $id)
			{
				foreach($this->get_newsletters(false,$id) as $item)
				{
					...
					{
						$this->db->where('id',$id)->update(self::MAIL,array('active'=>0));
					...
					elseif($item->active==0)
					{
						...
						{
							$this->db->where('id',$id)->update(self::MAIL,array('active'=>1));
							if(!isset($action)) $action='restore';
						}
						else//delete
						{
							$this->db->where('id',$id)->delete(self::MAIL);
							if(!isset($action)) $action='delete';
						}
					}
					$items[] = $item->subject;
		...
		elseif($preview===false)
		{
			...
			if($this->input->post('group'))
			{
				foreach($this->input->post('group') as $group_id)
				{
					...
					$this->db->join(self::USERS_GROUPS,self::USERS_GROUPS.'.user_id = '.self::USERS.'.id');
					$this->db->where('group_id',$group_id);
					$query=$this->db->get();
